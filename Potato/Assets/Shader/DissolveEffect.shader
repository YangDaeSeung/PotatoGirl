Shader "Dissolve Effect/Dissolving"
{
    Properties
    {
    ...
    _Edge("Edge",Range(0.01,0.5)) = 0.01
        ...
        [Header(Effect)]
    [Toggle(EDGE_COLOR)] _UseEdgeColor("Effect Use", Float) = 1
...
#include "UnityCG.cginc"
    ...
    v2f vert(appdata v)
    {
        ...
        o.uv = TRANSFORM_TEX(v.uv, _MainTex); // 메인 텍스쳐 트랜스폼 정보
        o.uv3 = TRANSFORM_TEX(v.uv, _DissolveTex); //Dissolve효과의 텍스쳐 트랜스폼 정보
        o.color = v.color;
        o.color.a *= _Progress;
        return o;
    }
